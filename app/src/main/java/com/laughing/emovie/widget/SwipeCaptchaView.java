...
import com.laughing.emovie.utils.DrawHelperUtils;
...
public class SwipeCaptchaView extends ImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMaskShadowPaint.setColor(Color.WHITE);
/*        mMaskShadowPaint.setStrokeWidth(50);
    ...
    private void createCaptchaPath() {
        ...
        if (mCaptchaX < mCaptchaWidth) {
            mCaptchaX = mCaptchaWidth + gap;
        }
...
/*        RectF oval = new RectF(mCaptchaX + gap, mCaptchaY - (r), mCaptchaX + gap + r * 2, mCaptchaY + (r));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isMatchMode) {
            ...
            if (null != mMaskBitmap && null != mMaskShadowBitmap && isDrawMask) {
                // 先绘制阴影
                canvas.drawBitmap(mMaskShadowBitmap, -mCaptchaX + mDragerOffset + mCaptchaWidth / 3, 0, mMaskShadowPaint);
                canvas.drawBitmap(mMaskBitmap, -mCaptchaX + mDragerOffset + mCaptchaWidth / 3, 0, null);
            }
    ...
    public void matchCaptcha() {
        if (null != onCaptchaMatchCallback && isMatchMode) {
            ...
            if (Math.abs(mDragerOffset - mCaptchaX + mCaptchaWidth / 3) < mMatchDeviation) {
                //matchSuccess();
                //成功的动画
                mSuccessAnim.start();
            } else {
    ...
    public int getMaxSwipeValue() {
        ...
        //返回控件宽度
        return mWidth - mCaptchaWidth - (mCaptchaWidth / 3) * 2;
    }
